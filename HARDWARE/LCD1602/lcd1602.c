...
void GPIO_Configuration(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOC,ENABLE);//使能PB,PD端口时钟
...
void LCD1602_Wait_Ready(void)
{
	...
	do
	{
		...
		delay_ms(5);	//延时5ms，非常重要
 		sta = GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_15);//读取状态字
		LCD_EN_Clr();
	}while(sta & 0x80);//bit7等于1表示液晶正忙，重复检测直到其等于0为止
}

/* 向LCD1602液晶写入一字节命令，cmd-待写入命令值 */
void LCD1602_Write_Cmd(u32 cmd)//u8
{
	cmd=cmd<<8;
	LCD1602_Wait_Ready();
	LCD_RS_Clr();
...
void LCD1602_Write_Dat(u32 dat)
{
	dat=dat<<8;
	LCD1602_Wait_Ready();
	LCD_RS_Set();
